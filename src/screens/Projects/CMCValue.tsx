...
function CMCValueScreen(props) {
  ...
  const queryResponse = useQuery(GET_CMCVALUE_DETAILS, {
    variables: {
      count,
      offset,
      projectId
  ...
  let CMCValueData = resp.payload.data;
  let projectView = CMCValueData.projectView;

  const valueListBlock = data => {
    return (
      <FlatList
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {CMCValueData.values.length > 0 ? (
        <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
          <Text style={st.valueNotes}>CMC Value Notes:</Text>
          {valueListBlock(CMCValueData.values)}
        </ScrollView>
      ) : (
        <Text style={st.noNotes}>No CMC Value notes</Text>
      )}
    </View>
  );
}
