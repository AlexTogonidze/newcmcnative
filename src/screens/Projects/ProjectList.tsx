    ...
    await queryResponse.fetchMore({
      ...
      updateQuery: (prev, { fetchMoreResult }) => {
        ...
        var merged = {
          ...
          payload: {
            ...prev.payload,
            data: prev.payload.data.concat(fetchMoreResult.payload.data)
          }
        };
        return merged;
      }
        ...
        <View>
          ...
          {selectedItem && (
            <View style={st.innerContainer}>
              <Text style={{textTransform: "capitalize"}}>{getLabel(selectedItem).replace(/([A-Z])/g, ' $1').replace(/^./, function(str){ return str.toUpperCase()}) }</Text>
  ...
  const optionTemplate = settings => {
    ...
    return (
      <View style={st.labelContainer}>
        <Text style={st.optionLabel}>{getLabel(item).replace(/([A-Z])/g, ' $1').replace(/^./, function(str){ return str.toUpperCase(); })}</Text>
      </View>
    );
  };
        ...
        renderItem={({ item, index }) => (
          ...
          >
            <View style={st.picContainer}>
              ...
              <Text style={st.score}><Image source={require("../../../assets/img/star.png")} style={{width: 13, height: 13}} /> {item.overall}</Text>
            </View>
